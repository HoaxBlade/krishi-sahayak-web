...
export const INDIAN_REGIONS: RegionData[] = [
  // North India
  {
    state: "Delhi",
...
// Helper functions
export const getStatesByRegion = (region: string): string[] => {
...
export const getDistrictsByState = (state: string): string[] => {
  const regionData = INDIAN_REGIONS.find(r => r.state === state);
...
export const getAllRegions = (): string[] => {
  return [...new Set(INDIAN_REGIONS.map(r => r.region))];
};

export const getAllStates = (): string[] => {
  return INDIAN_REGIONS.map(r => r.state);
};
